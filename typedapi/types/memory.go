...
// Attributes set the Attributes property for MemoryBuilder.
func (rb *MemoryBuilder) Attributes(value map[string]string) *MemoryBuilder {
	rb.v.Attributes = value
	return rb
}

// EphemeralId set the EphemeralId property for MemoryBuilder.
...
// Jvm Contains Java Virtual Machine (JVM) statistics for the node.
func (rb *MemoryBuilder) Jvm(jvm JvmStats) *MemoryBuilder {
	rb.v.Jvm = jvm
	return rb
}

// Mem Contains statistics about memory usage for the node.
func (rb *MemoryBuilder) Mem(mem MemStats) *MemoryBuilder {
	rb.v.Mem = mem
	return rb
}
