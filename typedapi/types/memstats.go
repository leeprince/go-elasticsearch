...
func NewMemStats() *MemStatsBuilder {
	r := MemStatsBuilder{
		&MemStats{},
	}
...
// then this reports the overridden value. Otherwise it reports the same value
// as total.
func (rb *MemStatsBuilder) AdjustedTotal(adjustedtotal ByteSize) *MemStatsBuilder {
	rb.v.AdjustedTotal = &adjustedtotal
	return rb
}
...
// Ml Contains statistics about machine learning use of native memory on the node.
func (rb *MemStatsBuilder) Ml(ml MemMlStats) *MemStatsBuilder {
	rb.v.Ml = ml
	return rb
}

// Total Total amount of physical memory.
func (rb *MemStatsBuilder) Total(total ByteSize) *MemStatsBuilder {
	rb.v.Total = &total
	return rb
}
