...
func NewDetectionRule() *DetectionRuleBuilder {
...
// a non-empty scope or at least one condition. Multiple conditions are combined
// together with a logical AND.
func (rb *DetectionRuleBuilder) Conditions(conditions ...RuleCondition) *DetectionRuleBuilder {
	rb.v.Conditions = append(rb.v.Conditions, conditions...)
	return rb
}
...
// Scoping is allowed for any of the fields that are also specified in
// `by_field_name`, `over_field_name`, or `partition_field_name`.
func (rb *DetectionRuleBuilder) Scope(value map[Field]FilterRef) *DetectionRuleBuilder {
	rb.v.Scope = value
	return rb
}
