...
func NewDelayedDataCheckConfig() *DelayedDataCheckConfigBuilder {
...
// In particular, the default `check_window` span calculation is based on the
// maximum of `2h` or `8 * bucket_span`.
func (rb *DelayedDataCheckConfigBuilder) CheckWindow(checkwindow Time) *DelayedDataCheckConfigBuilder {
	rb.v.CheckWindow = &checkwindow
	return rb
}
