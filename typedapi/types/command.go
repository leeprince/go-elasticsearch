...
// implications are well-understood, this command requires the flag
// accept_data_loss to be explicitly set to true.
func (rb *CommandBuilder) AllocateEmptyPrimary(allocateemptyprimary CommandAllocatePrimaryAction) *CommandBuilder {
	rb.v.AllocateEmptyPrimary = &allocateemptyprimary
	return rb
}
...
// index name and shard number, and node to allocate the shard to. Takes
// allocation deciders into account.
func (rb *CommandBuilder) AllocateReplica(allocatereplica CommandAllocateReplicaAction) *CommandBuilder {
	rb.v.AllocateReplica = &allocatereplica
	return rb
}
...
// well-understood, this command requires the flag accept_data_loss to be
// explicitly set to true.
func (rb *CommandBuilder) AllocateStalePrimary(allocatestaleprimary CommandAllocatePrimaryAction) *CommandBuilder {
	rb.v.AllocateStalePrimary = &allocatestaleprimary
	return rb
}
...
// a primary shard then the allow_primary flag must also be included in the
// request.
func (rb *CommandBuilder) Cancel(cancel CommandCancelAction) *CommandBuilder {
	rb.v.Cancel = &cancel
	return rb
}
...
// for index name and shard number, from_node for the node to move the shard
// from, and to_node for the node to move the shard to.
func (rb *CommandBuilder) Move(move CommandMoveAction) *CommandBuilder {
	rb.v.Move = &move
	return rb
}
